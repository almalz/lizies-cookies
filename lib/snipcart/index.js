...
export const SnipcartLayout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && !isCartOpen) {
      ...
      console.log('false')
      Array.from(
        document.querySelectorAll('#snipcart-shipping-address-form label')
      ).forEach((el) => (el.style.color = '#2e3338'))
    }
  }, [isCartOpen])

  //snipcart__font--tiny snipcart-form__label--checkbox

  // Add terms acceptation text before payement button
  useEffect(() => {
    if (typeof window !== 'undefined' && isCartOpen) {
      const message = document.createElement('p')
      message.innerText =
        'En cliquant sur le bouton ci-dessous, vous acceptez nos CGV.'
      ...
      if (checkoutButton) {
        checkoutButton.parentNode.insertBefore(message, checkoutButton)
      }
    }
  }, [isCartOpen])
